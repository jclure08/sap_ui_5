...
sap.ui.define([
	...
	"sap/m/MessageBox",
	"sap/m/MessageToast"
], function (BaseController, JSONModel, core_formatter, MessageBox, MessageToast) {
	"use strict";

	return BaseController.extend("com.webstersys.surt.user.controller.Detail", {
		...
		onInit: function () {
			...
			var oViewModel = new JSONModel({
				...
				mode: "display",
				delay: 0,
				authorityListTitle: this.getResourceBundle().getText("authorityTableHeading"),
				subordinatesListTitle: this.getResourceBundle().getText("subordinatesTableHeading")
			});
		...
		onListUpdateFinished: function (oEvent) {
			...
			if (this.byId("userAuthorities").getBinding("items").isLengthFinal()) {
				if (iTotalItems) {
					sTitle = this.getResourceBundle().getText("authorityTableHeadingCount", [iTotalItems]);
				} else {
					//Display 'Line Items' instead of 'Line items (0)'
					sTitle = this.getResourceBundle().getText("authorityTableHeading");
				}
				oViewModel.setProperty("/authorityListTitle", sTitle);
			}
		},
		onSubordinatesListUpdateFinished: function (oEvent) {
			var sTitle,
				iTotalItems = oEvent.getParameter("total"),
				oViewModel = this.getModel("detailView");

			// only update the counter if the length is final
			if (this.byId("subordinates").getBinding("items").isLengthFinal()) {
				if (iTotalItems) {
					sTitle = this.getResourceBundle().getText("subordinatesTableHeadingCount", [iTotalItems]);
				} else {
					//Display 'Line Items' instead of 'Line items (0)'
					sTitle = this.getResourceBundle().getText("subordinatesTableHeading");
				}
				oViewModel.setProperty("/subordinatesListTitle", sTitle);
			}
		},
		...
		onDelete: function () {
			...
			var oViewModel = this.getModel("detailView"),
				...
				sObjectHeader = this._oODataModel.getProperty(sPath + "/FirstName") + " "+this._oODataModel.getProperty(sPath + "/LastName"),
			...
			var fnGotSubordinateCount = function (count) {
				oViewModel.setProperty("/busy", false);
				if (count > 0) {
					MessageBox.error("You can not delete account.Please remove/assign subordinate to another account");
					return;
				} else {
			...
			var fnSubordinateCountFailed = function () {
				oViewModel.setProperty("/busy", false);
			};
			oViewModel.setProperty("/busy", true);
			this._oODataModel.read(sPath + "/Subordinate/Users/$count", {
				success: fnGotSubordinateCount.bind(this),
				error: fnSubordinateCountFailed.bind(this)
			});
		...
		onEdit: function () {
			...
			this.getRouter().getTargets().display("edit", {
				Id: encodeURIComponent(oObject.Id)
			});
		},
		...
		_onMetadataLoaded: function () {
			...
			var iOriginalViewBusyDelay = this.getView().getBusyIndicatorDelay(),
				oViewModel = this.getModel("detailView"),
				userAuthoritiesTable = this.byId("userAuthorities"),
				subordinatesTable = this.byId("subordinates"),
				iOriginalSubordinatesTableBusyDelay = subordinatesTable.getBusyIndicatorDelay(),
				iOriginalUserAuthoritiesTableBusyDelay = userAuthoritiesTable.getBusyIndicatorDelay();

			// Make sure busy indicator is displayed immediately when
			// detail view is displayed for the first time
			oViewModel.setProperty("/delay", 0);
			oViewModel.setProperty("/userAuthoritiesTableDelay", 0);
			oViewModel.setProperty("/subordinatesTableDelay", 0);

			userAuthoritiesTable.attachEventOnce("updateFinished", function () {
				// Restore original busy indicator delay for line item table
				oViewModel.setProperty("/userAuthoritiesTableDelay", iOriginalUserAuthoritiesTableBusyDelay);
			});
			subordinatesTable.attachEventOnce("updateFinished", function () {
				// Restore original busy indicator delay for line item table
				oViewModel.setProperty("/subordinatesTableDelay", iOriginalSubordinatesTableBusyDelay);
			});
